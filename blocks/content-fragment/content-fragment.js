...
function renderUI(data = []) {
    ...
    function applyDepartureFilter(from, to) {
        ...
        if (from && to) {
            ...
            filteredData = data.filter(card =>
                card.destinationCity && card.destinationCity.toLowerCase().includes(to.trim().toLowerCase())
            );
        } else {
            filteredData = data;
        }
    ...
    function departurePopover(target) {
        ...
        function createInput(id, value, placeholder, descId, onInput, suggestionsId) {
            ...
            function showSuggestions(val) {
                let filtered = cityOptions.filter(opt => opt.toLowerCase().includes(val.trim().toLowerCase()));
        ...
        popover = div({ class: 'popover-body', style: 'position:absolute;z-index:1000;top:0;left:0;background:#fff;box-shadow:0 2px 8px rgba(0,0,0,0.15);padding:16px;border-radius:8px;min-width:320px;' },
            div({ class: 'route-popover' },
                ...
                div({ class: 'popover-body-content' },
                    div({ class: 'popover-fields-wrapper' },
                        div({ class: 'popover-fields-row' },
                            div({ class: 'location-selector-container fr-position-relative from-field' },
                                div({
                                    class: 'fr-label fr-ml-0 fr-pt-0 fr-text-text fr-text-left fr-w-100 fr-d-inline-block',
                                    id: fromLabelId,
                                    for: fromInputId
                                }, 'From'),
                                div({ class: 'fr-bg-white fr-border fr-border-borders' },
                                    div({ class: 'fr-input fr-input-group fr-w-100' },
                                        div({ 'aria-hidden': 'true', class: 'fr-input-group-prepend' },
                                ...
                                div({ as: 'p', id: fromDescId, class: 'fr-sr-only', 'aria-hidden': 'true' }, 'Enter the origin city of the trip.')
                            ),
                            div({ class: 'location-selector-container fr-position-relative to-field' },
                                div({
                                    class: 'fr-label fr-ml-0 fr-pt-0 fr-text-text fr-text-left fr-w-100 fr-d-inline-block',
                                    id: toLabelId,
                                    for: toInputId
                                ...
                                div({ as: 'p', id: toDescId, class: 'fr-sr-only', 'aria-hidden': 'true' }, 'Enter the destination city of the trip.')
                            )
                        )
                    )
                ),
                ...
                div({ class: 'popover-footer' },
                    div({ as: 'button', disabled: false, class: 'popover-btn', onclick: onClear }, 'Clear'),
                    div({ as: 'button', class: 'popover-btn', onclick: onDone, disabled: (from && to && from.trim().toLowerCase() === to.trim().toLowerCase()) }, 'Done')
                )
            )
        );
            ...
            div({ class: 'fr-secondary-filters fr-d-flex' },
                div({ class: 'fr-d-inline-block' },
                    div({ class: 'fr-filters-left' },
                        ...
                        div({ class: 'fr-btn-clear-wrapper' },
                            div({
                                as: 'button',
                ...
                div({ class: 'fr-sorting-filter-column fr-ml-auto' },
                    div({ class: 'fr-sortby-wrapper' },
                        div({
                            as: 'button',
