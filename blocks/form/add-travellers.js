...
function ADD_TRAVELLERS(block) {
  ...
  function updateTravellerData() {
    ...
    const travellers = storeTravellerData.travellers;
    const nonZeroAgeGroups = Object.entries(travellers)
      .filter(([ageGroup, count]) => count > 0);
    debugger;
    const travelForm = document.getElementById("travel-details");
    travelForm.dataset.travellersAgeGroup = JSON.stringify(nonZeroAgeGroups);
  }
